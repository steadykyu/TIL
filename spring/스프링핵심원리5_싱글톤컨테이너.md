...
+ @Configuration을 설정정보 클래스에 입력해주면 Bean 컨테이너는 싱글톤 컨테이너로 만들어진다. 
+ 싱글톤 Bean 컨테이너는 동시에 오는 요청에 대해서, 공유하는 객체를 이용하여 메모리 낭비를 줄일 수 있다.
...
## @Configuration과 싱글톤
+ @Configuration를 설정정보클래스에 입력해주면, 스프링빈을 싱글톤타입으로 만들 수 있다.
...
+ 이 CGlib는 컨테이너속에서 이름은 appConfig이지만, 인스턴스 객체는 이 CGlib 클래스를 거친후 반환된 것을 쓰고 있을 것이다.
+ CGlib클래스가 어떻게 객체들을 생성하는지는 복잡하다.아마 반환 방식은 논리적으로 대략적으로는 다음과 같을 것이다.
```java
public class CGLIB{
    @Bean
    public MemberRepository memberRepository() {
